-- 070_shotgun_exclude_pm.sql
-- Geplante Wartung (PM-Ruf / "Wartung durchführen") NICHT als Shotgun zählen — auch dann
-- nicht, wenn das Ticket ein Symptomwort enthält ("Wartung durchführen - Streifen").
--
-- Problem: Der Symptom-Klassifikator stuft "Wartung durchführen - Streifen" als
-- Bildqualität ein (Symptom hat Vorrang), wodurch geplante Wartungen mit 3+ Teilen als
-- Shotgun gewertet wurden (~20 flottenweit, 12 bei Späth). Bei einer PM ist ein Teile-Kit
-- aber korrekt. Fix: shotgun_verdacht schließt PM-Marker im Freitext zusätzlich aus.
-- (Symptom-Spalte bleibt unverändert, damit die Symptom->Teil-Sicht erhalten bleibt.)
...
SELECT m.radix_activity_id, m.radix_ticket_id, m.occurred_at::date AS datum,
       ...
       (m.teiltypen >= 3
        ...
        -- geplante Wartung (PM) auch bei Symptomwort ausschliessen:
