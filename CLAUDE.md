...
Next: cost summary views (vw_cost_breakdown) + a cost/profitability UI tab; `profitability_snapshots` once click prices (`config/contract_pricing.yaml`) + labor rate are provided. Chat agent (Phase 4) needs `krai-ollama-prod` up. **Radix contract/cost import is gated on a pending user governance decision** (see `todo.md`). In-app chat agent = Phase 4 (Ollama; `krai-ollama-prod` was stopped at last check). See the plan + memory for the full roadmap.
