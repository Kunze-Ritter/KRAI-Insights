...
Next: Radix **cost crawl** (activities per customer → spareparts + sparepartprice + time → `cost_events`); then `profitability_snapshots` (revenue needs click prices — `config/contract_pricing.yaml`, still user input). Chat agent (Phase 4) needs `krai-ollama-prod` up. **Radix contract/cost import is gated on a pending user governance decision** (see `todo.md`). In-app chat agent = Phase 4 (Ollama; `krai-ollama-prod` was stopped at last check). See the plan + memory for the full roadmap.
